...
RGBAValue::RGBAValue(const float * buffer)
    :RGBAValue(buffer[0],buffer[1],buffer[2],buffer[3])
{
...
RGBAValue operator *(const RGBAValue &colour,float scalar){
    return RGBAValue(   scalar * (float) colour.red,
                        scalar * (float) colour.green,
                        scalar * (float) colour.blue,
                        scalar * (float) colour.alpha);
...
RGBAValue operator *(const RGBAValue &a,const RGBAValue &b){
        return {
            a.red * b.red / 255.f,
            a.green * b.green / 255.f,
            a.blue * b.blue / 255.f,
            a.alpha * b.alpha / 255.f
        };
}
